...
public class Code {
    //Prefijo de instrucción tipo C
    public static final String C_PRIFIX = "111";

    //Tabla de conversión a código binario (Dest, Comp & Jump)
    ...
    //Captura de excepciones de las expresiones DEST, COMP y JUMP
    public String dest(final String exp) {
        String ret = destTable.get(exp);
        if (ret == null) {
            throw new RuntimeException("No se puede encontrar la expresión DEST '" + exp + "'");
        }
        return ret;
    }
    public String comp(final String exp) {
        ...
        if (ret == null) {
            throw new RuntimeException("No se puede encontrar la expresión COMP '" + exp + "'");
        }
        return ret;
    }
    public String jump(final String exp) {
        ...
        if (ret == null) {
            throw new RuntimeException("No se puede encontrar la expresión JUMP '" + exp + "'");
        }
        return ret;
    }
